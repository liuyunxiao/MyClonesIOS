...
-(void)showNextMonth {
    ...
    [UIView animateWithDuration:.35
                     animations:^{
                         ...
                         if (hasNextMonthDays) {
                             animationView_A.frameX = -animationView_A.frameWidth + kVRGCalendarViewDayWidth+3;
                         } else {
...
-(void)showPreviousMonth {
    ...
    if (hasPreviousDays) {
        animationView_B.frameX = animationView_A.frameX - (animationView_B.frameWidth-kVRGCalendarViewDayWidth) + 3;
    } else {
    ...
    [UIView animateWithDuration:.35
                     animations:^{
                         ...
                         if (hasPreviousDays) {
                             animationView_A.frameX = animationView_B.frameWidth-(kVRGCalendarViewDayWidth+3);
...
{       
    ...
    if (touchPoint.y > kVRGCalendarViewTopBarHeight+10) {
        float xLocation = touchPoint.x;
        float yLocation = touchPoint.y-kVRGCalendarViewTopBarHeight-10;
    ...
    CGRect rectArrowLeft = CGRectMake(0, 20, 50, 40);
    CGRect rectArrowRight = CGRectMake(self.frame.size.width-50, 20, 50, 40);
...
{
    int firstWeekDay = [self.currentMonth firstWeekDayInMonth]; //如果-1的话 because weekdays begin at 1, not 0
    ...
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"f8f4f1"].CGColor);
...
//    CGContextFillPath(context);
    ...
    UIImage *image1 = [UIImage imageNamed:@"zj.png"];
    CGRect rect1 = CGRectMake(xmargin, ymargin, image1.size.width/1.5, image1.size.height/1.5);
    CGContextDrawImage(context, rect1, [image1 CGImage]);
...
//    CGContextFillPath(context);
    ...
    UIImage *image2 = [UIImage imageNamed:@"yj.png"];
    CGRect rect2 = CGRectMake(self.frame.size.width-(xmargin+image2.size.width/1.5), ymargin, image2.size.width/1.5, image2.size.height/1.5);
    CGContextDrawImage(context, rect2, [image2 CGImage]);
...
//    [weekdays moveObjectFromIndex:0 toIndex:6];
    ...
    for (int i =0; i<[weekdays count]; i++) {
        ...
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth), 70, kVRGCalendarViewDayWidth, 20) withFont:font lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
    ...
    CGContextMoveToPoint(context, 0.0, 95.0);
    CGContextAddLineToPoint(context, 310.0,95.0);
    CGContextStrokePath(context);
//    CGContextClosePath(context);
    
    //Grid background
    float gridHeight = (numRows+1)*(kVRGCalendarViewDayHeight)+1;
    CGRect rectangleGrid = CGRectMake(0,kVRGCalendarViewTopBarHeight,self.frame.size.width,gridHeight+50);
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (i<firstWeekDay) { //previous month
            ...
            UIColor *color = (isSelectedDatePreviousMonth) ? [UIColor whiteColor] : [UIColor lightGrayColor];
...
//            NSString *hex = (isSelectedDatePreviousMonth || isSelectedDateNextMonth) ? @"0xaaaaaa" : @"0x383838";
            ...
            UIColor *color = (isSelectedDatePreviousMonth) ? [UIColor colorWithHexString:@"0xaaaaaa"] : [UIColor darkGrayColor];
        ...
        if (selectedDate && i==selectedDateBlock) {
            CGContextAddArc(context, targetX+kVRGCalendarViewDayWidth/2-1, targetY+kVRGCalendarViewDayHeight/2-1+10, 11, 0, 360, 0);
            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"f5ad01"].CGColor);
            CGContextFillPath(context);
            
            CGContextSetFillColorWithColor(context, 
                                           [UIColor whiteColor].CGColor);
        } else if (todayBlock==i) {//今天的颜色
//            CGRect rectangleGrid = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth,kVRGCalendarViewDayHeight);
//            CGContextAddRect(context, rectangleGrid);
            CGContextAddArc(context, targetX+kVRGCalendarViewDayWidth/2-2, targetY+kVRGCalendarViewDayHeight/2-2+10, 11, 0, 360, 0);
        ...
        [date drawInRect:CGRectMake(targetX, targetY+20, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont systemFontOfSize:16] lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        ...
        int targetX = targetColumn * (kVRGCalendarViewDayWidth) + 7;
        int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight) + 38;
...
-(UIImage *)drawCurrentState {
    float targetHeight = kVRGCalendarViewTopBarHeight + [self numRows]*(kVRGCalendarViewDayHeight)+1+50;
    
    UIGraphicsBeginImageContext(CGSizeMake(kVRGCalendarViewWidth, targetHeight-kVRGCalendarViewTopBarHeight+50));
...
#pragma mark - Init
-(id)init {
    self = [super initWithFrame:CGRectMake(15, 10, kVRGCalendarViewWidth, 0)];
    if (self) {
        ...
        labelCurrentMonth.font = [UIFont systemFontOfSize:20];
        labelCurrentMonth.textColor = [UIColor colorWithHexString:@"f5ad01"];
