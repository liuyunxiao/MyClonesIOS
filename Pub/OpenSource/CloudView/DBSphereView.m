...
- (void)setTagOfPoint: (DBPoint)point andIndex:(NSInteger)index
{
    ...
    CGFloat offx=fabsf(view.center.x-105.f);
    CGFloat offy=fabsf(view.center.y-105.f);
//    CGFloat transform = (point.z + 2) / 3;
    if(view.layer.zPosition<0.9)
    {
        if(offx>offy)
        {
            offx=-offx;
    ...
    CGFloat transform = (point.z + 2) / 3+offx/1000*3;
    //
...
//    NSLog(@"button:%@ cx:%f cy:%f z:%f",((UIButton*)view).titleLabel.text,view.center.x,view.center.y,view.layer.zPosition);
}
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        aniTimer=[NSTimer scheduledTimerWithTimeInterval:1.f target:self selector:@selector(timeStop) userInfo:nil repeats:NO];
