...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        headersCache = [[NSMutableDictionary alloc] init];
...
- (void)dealloc
{
    ...
    [headersCache release],headersCache=nil;
...
- (NSString *)cacheHeaderPathForKey:(NSString *)key
{
    const char *str = [key UTF8String];
    unsigned char r[CC_MD5_DIGEST_LENGTH];
    CC_MD5(str, (CC_LONG)strlen(str), r);
    ...
    return [diskCachePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.header",filename]];
}
...
//存储下载文件Http头文件信息，解决相同url不能更新图片的问题
//add by Jeerain
//2013-08-21
- (void)storeHeaderField:(NSDictionary *)headerDic forKey:(NSString *)key toDisk:(BOOL)toDisk
{
    if(toDisk)
    {
        NSDateFormatter* dateFormat = [[NSDateFormatter alloc] init];
        [dateFormat setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        NSString *currentDateStr = [dateFormat stringFromDate:[[NSDate date] addTimeInterval:900]];
        NSMutableDictionary *dic=[[NSMutableDictionary alloc] initWithDictionary:headerDic];
        [dic setObject:currentDateStr forKey:@"Expires"];
        BOOL flag=[dic writeToFile:[self cacheHeaderPathForKey:key] atomically:NO];
        NSLog(@"store header is %d",flag);
        
        [headersCache setObject:dic forKey:key];
    }
}

- (NSDictionary *)headerFromKey:(NSString *)key
{
    if (key == nil)
    {
        return nil;
    ...
    NSMutableDictionary *ch=[headersCache objectForKey:key];
    if(ch==nil)
    {
        ch = [NSMutableDictionary dictionaryWithContentsOfFile:[self cacheHeaderPathForKey:key]];
        if(ch!=nil)
            [headersCache setObject:ch forKey:key];
    }
    return ch;
}
