...
- (NSDictionary *)cacheHeaderForKey:(NSString *)url
{
    NSDictionary *cachedHeaders=[[SDImageCache sharedImageCache] headerFromKey:url];
    return cachedHeaders;
}

- (void)storeHeaderField:(NSDictionary *)headerDic forKey:(NSString *)key
{
    [[SDImageCache sharedImageCache] storeHeaderField:headerDic forKey:key toDisk:YES];
}
...
//add by Jeerain
//2013-08-21
- (void)storeHttpHeaderFields:(SDWebImageDownloader *)downloader headeFields:(NSDictionary*)headerDic
{
    if (headerDic)
    {
        SDWebImageOptions options = [[downloader.userInfo objectForKey:@"options"] intValue];
        [[SDImageCache sharedImageCache] storeHeaderField:headerDic
